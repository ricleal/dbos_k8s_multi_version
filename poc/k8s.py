...
Read-only, and pods only. This module answers one question — "is the executor
that claimed this row still there" — and nothing else. Retiring a version is a
separate decision that reads Deployments and deletes them, and it belongs to an
operator outside the cluster: ``make retire``, run from cron. So the
ServiceAccount needs ``get`` and ``list`` on pods, and nothing more. This app
